...
test("HypertuneProvider", async () => {
  ...
  // Simple flags.
  expect(await client.getBooleanValue("booleanFlag", false)).toBe(true);
  expect(await client.getStringValue("stringFlag", "Fallback")).toBe(
    "TopLevelString"
  );
  expect(await client.getStringValue("enumFlag", "Fallback")).toBe("test");
  ...
  expect(await client.getObjectValue("group", {})).toMatchInlineSnapshot(`
    {
      "__typename": "Group",
      "booleanFlag": true,
      "enumFlag": "test",
